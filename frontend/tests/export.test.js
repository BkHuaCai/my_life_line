...
describe('export/import', () => {
  it('serialize 输出嵌套结构并保留 id', async () => {
    ...
    const grown = xm.timelines.find((t) => t.name === '成长')
    expect(grown.events[0].title).toBe('毕业')
  ...
  it('round-trip：导出后再导入新库，数据等价', async () => {
    ...
    const grown = tls.find((t) => t.name === '成长')
    expect(grown).toBeTruthy()
    const evs = await db2.getEventsByTimeline(grown.id)
  ...
  it('重复导入不产生重复数据（按 id 幂等）', async () => {
    ...
    const grown = tls.find((t) => t.name === '成长')
    expect((await db.getEventsByTimeline(grown.id)).length).toBe(1)
  })
})
